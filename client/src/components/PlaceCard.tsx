...
import { Star, Clock, Car, Sun, Cloud, CloudRain, Snowflake, CloudLightning, CloudSun, Droplets } from "lucide-react";
          ...
          {weather && (
            <div className="flex items-center gap-1 text-[10px] sm:text-xs font-medium text-white/90 bg-black/40 backdrop-blur-sm px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md">
              ...
              {weather.precipProb > 0 && (
                <>
                  <Droplets className="w-2 sm:w-2.5 h-2 sm:h-2.5 text-blue-300" />
                  <span>{weather.precipProb}%{getPrecipType(weather.code) ? ` ${getPrecipType(weather.code)}` : ''}</span>
                </>
              )}
            </div>
          )}
